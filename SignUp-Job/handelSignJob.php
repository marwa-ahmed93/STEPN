...
$cause_reason =$_POST['cause_reason'];
...
$cause_reason=cleanData($cause_reason);
...
// validate cause_reason
if (empty($cause_reason) || !filter_var($cause_reason,  FILTER_SANITIZE_STRING)) {
  $errors['cause_reason'] = "enter valid cause_reason";
  $is_valid=false;
}

// validate specialization
if (empty($specialization) || !filter_var($specialization,  FILTER_SANITIZE_STRING)) {
    $errors['specialization'] = "enter valid specialization";
    $is_valid=false;
  }
//validate wCountry
...
//validate gender
if(empty($wsex) || !filter_var($wsex , FILTER_SANITIZE_STRING))
{
    $errors['wsex'] = "enter valid gender";
    $is_valid=false;
}
//validate email
...
{
    header('location: ../SignUp-job/SignUp.php');
}

if($is_valid===true)
{
    //insert data
    $query ="INSERT INTO `new_official`(`first_name`, `laste-name`, `user_name`, `phone`, `birthday`,
     `cause_reason`, `specialization`, `country`, `governorate`, `city`, `national_id`, `gender`, `national_states`, `email`, `password`) VALUES 
     ('$wfirstname','$wlastname','$wUsername',$wPhone,'$birthday','$cause_reason','$specialization'
 '$wCountry','$governorate','$city',$national_id,'$gender','$wsex','$wEmail','$wpassword')
    ";
